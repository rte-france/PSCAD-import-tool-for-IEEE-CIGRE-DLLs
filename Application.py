...
class Application(tk.Tk):
    ...
    def __init__(self, num_version):
        ...
        self.DLLInterfaceVersionStr = None
    ...
    def generate_pscad_model(self):
        ...
        try:
            ...
            self.get_dll_interface_version_str()
    ...
    def create_fortran_code(self, type_modelinputs, type_modeloutputs, type_modelparameters,
        ...
        bf += '\tIF (FIRST_CALL_THIS_FILE) THEN  ! Use FIRST_CALL_THIS_FILE to do following instruction once\n\n' \
              ...
              '\t\tENDIF\n\n' \
              '\t\tDLLModelVersion_cp = Model_Info_fp.ModelVersion\n' \
              '\t\tIF (Get_Fortran_String(DLLModelVersion_cp) .NE. OrigModelVersion ) THEN\n' \
              '\t\t\tWRITE(IUNIT,*) "*** Error - The model version in the fortran interface code generated by the DLLImport tools is: ", OrigModelVersion\n' \
              '\t\t\tWRITE(IUNIT,*) "but the model version in the DLL is: ", Get_Fortran_String(Model_Info_fp.ModelVersion)\n' \
              '\t\t\tWRITE(IUNIT,*) "They must match."\n' \
              '\t\tENDIF\n\n' \
    ...
    def get_dll_interface_version_str(self):
