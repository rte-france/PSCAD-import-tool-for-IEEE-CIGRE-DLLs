...
class Application(tk.Tk):
    ...
    # For inputs, outputs or parameters whose type is not string
    ...
    def generate_conversion(self, names, widths, fortran_types, pscad_types, suffix, fill_new_struct, new_struct):
        ...
        for i in range(0, len(names)):
            ...
            if pscad_type == 'CHARACTER(*)':  # string not handled
                continue
    ...
    def create_fortran_code(self):
        ...
        bf += '\t! Common module for each instance of ' + self.Model_Name_Shortened + ' model\n' \
              ...
              '\tUSE, INTRINSIC :: iso_c_binding, only : c_ptr, c_double, c_int8_t, c_int32_t, c_null_char\n' \
        ...
        # if string parameters are used, declare their "C-format" variables
        ...
        for i in range(len(self.param_names)):
            ...
            if param_pscad_type == 'CHARACTER(*)':
                has_string_param = True
                bf += '\tCHARACTER(LEN=LEN_TRIM(' + self.param_names[i] + '_pscad)+1), TARGET :: ' + self.param_names[i] + '_pscad_c_format\n'
        if has_string_param:
            bf += '\n'
        ...
        # Conversion for all parameters except string parameters
        ...
        for i in range(len(self.param_names)):
            param_pscad_type = self.param_pscad_types[
                i]  # For an IEEE CIGRE DLL, it can be INTEGER, REAL or CHARACTER(*)
            if param_pscad_type == 'CHARACTER(*)':
                bf += '\t' + self.param_names[i] + '_pscad_c_format = TRIM(' + self.param_names[i] + '_pscad) // C_NULL_CHAR\n'
                bf += '\tPARAMETERS_new%' + self.param_names[i] + ' = C_LOC(' + self.param_names[i] + '_pscad_c_format)\n\n'
    ...
    def generate_pscad_project(self):
        ...
        for i in range(len(self.param_names)):
            ...
            if param_pscad_type == 'INTEGER':
                ...
                model_parameters.text(param_name, description=param_description, value='"'+str(self.param_default_values[i])+'"',
                                      group=param_group_name)
            else:
